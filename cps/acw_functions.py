...
acw_stats = Blueprint('acw_stats', __name__)
acw_check_status = Blueprint('acw_check_status', __name__)
acw_settings = Blueprint('acw_settings', __name__)
acw_logs = Blueprint('acw_logs', __name__)
...
@csrf.exempt
@library_refresh.route("/acw-library-refresh", methods=["GET", "POST"])
@login_required_if_no_ano
def acw_library_refresh():
...
@csrf.exempt
@library_refresh.route("/acw-library-refresh/messages", methods=["GET"])
@login_required_if_no_ano
...
@csrf.exempt
@acw_settings.route("/acw-settings", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def set_acw_settings():
    acw_db = ACW_DB()
    acw_default_settings = acw_db.acw_default_settings
        ...
        if request.form['submit_button'] == "Submit":
            ...
            for setting in string_settings:
                ...
                if setting[:14] == "ignore_convert":
                    ...
                    value = acw_db.acw_settings['auto_convert_target_format']
...
def get_acw_stats() -> dict[str,int]:
    """Returns ACW stat totals as a dict (keys are table names except for total_books)"""
    acw_db = ACW_DB()
    totals = acw_db.get_stat_totals()
    totals["total_books"] = acw_get_num_books_in_library() # from web.py
...
@acw_stats.route("/acw-stats-show", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def acw_stats_show():
    acw_db = ACW_DB()
    data_enforcement = acw_db.enforce_show(paths=False, verbose=False, web_ui=True)
    data_enforcement_with_paths = acw_db.enforce_show(paths=True, verbose=False, web_ui=True)
    data_imports = acw_db.get_import_history(verbose=False)
    data_conversions = acw_db.get_conversion_history(verbose=False)
...
@acw_stats.route("/acw-stats-show/full-enforcement", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def show_full_enforcement():
    acw_db = ACW_DB()
    data = acw_db.enforce_show(paths=False, verbose=True, web_ui=True)
    return render_title_template("acw_stats_full.html", title=_("Autocaliweb - Full Enforcement History"), page="acw-stats-full",
                                    table_headers=headers["enforcement"]["no_paths"], data=data)

@acw_stats.route("/acw-stats-show/full-enforcement-with-paths", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def show_full_enforcement_path():
    acw_db = ACW_DB()
    data = acw_db.enforce_show(paths=True, verbose=True, web_ui=True)
    return render_title_template("acw_stats_full.html", title=_("Autocaliweb - Full Enforcement History (w/ Paths)"), page="acw-stats-full",
                                    table_headers=headers["enforcement"]["with_paths"], data=data)

@acw_stats.route("/acw-stats-show/full-imports", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def show_full_imports():
    acw_db = ACW_DB()
    data = acw_db.get_import_history(verbose=True)
    return render_title_template("acw_stats_full.html", title=_("Autocaliweb - Full Import History"), page="acw-stats-full",
                                    table_headers=headers["imports"], data=data)

@acw_stats.route("/acw-stats-show/full-conversions", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def show_full_conversions():
    acw_db = ACW_DB()
    data = acw_db.get_conversion_history(verbose=True)
    return render_title_template("acw_stats_full.html", title=_("Autocaliweb - Full Conversion History"), page="acw-stats-full",
                                    table_headers=headers["conversions"], data=data)

@acw_stats.route("/acw-stats-show/full-epub-fixer", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def show_full_epub_fixer():
    acw_db = ACW_DB()
    data = acw_db.get_epub_fixer_history(fixes=False, verbose=True)
    return render_title_template("acw_stats_full.html", title=_("Autocaliweb - Full EPUB Fixer History (w/out Paths & Fixes)"), page="acw-stats-full",
                                    table_headers=headers["epub_fixer"]["no_fixes"], data=data)

@acw_stats.route("/acw-stats-show/full-epub-fixer-with-paths-fixes", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def show_full_epub_fixer_with_paths_fixes():
    acw_db = ACW_DB()
    data = acw_db.get_epub_fixer_history(fixes=True, verbose=True)
    return render_title_template("acw_stats_full.html", title=_("Autocaliweb - Full EPUB Fixer History (w/ Paths & Fixes)"), page="acw-stats-full",
...
@acw_check_status.route("/acw-check-monitoring", methods=["GET", "POST"])
@login_required_if_no_ano
@admin_required
def acw_flash_status():
    ...
    match services_status:
        case 0:
            flash(_("✅ All Monitoring Services are running as intended! 👍"), category="acw_refresh")
        case 1:
            flash(_("🔴 The Ingest Service is running but the Metadata Change Detector is not"), category="acw_refresh")
        case 2:
            flash(_("🔴 The Metadata Change Detector is running but the Ingest Service is not"), category="acw_refresh")
        case 3:
            flash(_("⛔ Neither the Ingest Service or the Metadata Change Detector are running"), category="acw_refresh")
        case _:
            flash(_("An Error has occurred"), category="acw_refresh")
...
@acw_logs.route('/acw-logs/download/<log_filename>')
...
@acw_logs.route('/acw-logs/read/<log_filename>')
def read_log(log_filename):
    try:
        ...
        return render_title_template('acw_read_log.html', title=_(f"Autocaliweb - Log Archive - Read Log - {log_filename}"), page="acw-log-read",
...
##————————————————————————————————————————————————————————————————————————————##
##                                                                            ##
##                        ACW LIBRARY CONVERSION SERVICE                      ##
##                                                                            ##
##————————————————————————————————————————————————————————————————————————————##
...
def archive_run_log(log_path):
    try:
        ...
        print(f"[acw-functions] Log '{log_path}' has been successfully archived as {log_name} in '{LOG_ARCHIVE}'")
    except Exception as e:
        print(f"[acw-functions] The following error occurred when trying to back up {log_path} at {datetime.now()}:\n{e}")
...
@convert_library.route('/acw-convert-library-overview', methods=["GET"])
def show_convert_library_page():
    return render_title_template('acw_convert_library.html', title=_("Autocaliweb - Convert Library"), page="acw-library-convert",
                                target_format=ACW_DB().acw_settings['auto_convert_target_format'].upper())

@convert_library.route('/acw-convert-library/log-archive', methods=["GET"])
def show_convert_library_logs():
    logs=get_logs_from_archive("convert-library")
    log_dates = get_log_dates(logs)
    return render_title_template('acw_list_logs.html', title=_("Autocaliweb - Convert Library"), page="acw-library-convert-logs",
...
def kill_epub_fixer(queue):
    ...
    while True:
        ...
        if trigger_file.exists():
            ...
            # Add string to log to notify user of successful cancellation and to stop the JS update script
            with open(log_path, 'a') as f:
                f.write(f"\nACW EPUB FIXER PROCESS TERMINATED BY USER AT {datetime.now()}")
            # Add run log to log_archive
            archive_run_log(log_path)
            break
...
@epub_fixer.route('/acw-epub-fixer-overview', methods=["GET"])
def show_epub_fixer_page():
    return render_title_template('acw_epub_fixer.html', title=_("Autocaliweb - Send-to-Kindle EPUB Fixer Service"), page="acw-epub-fixer")

@epub_fixer.route('/acw-epub-fixer/log-archive', methods=["GET"])
def show_epub_fixer_logs():
    logs = get_logs_from_archive("epub-fixer")
    log_dates = get_log_dates(logs)
    return render_title_template('acw_list_logs.html', title=_("Autocaliweb - Send-to-Kindle EPUB Fixer Service"), page="acw-epub-fixer-logs",
                                logs=logs, log_dates=log_dates)
