...
/* This was adopted from the Calibre-Web Automated (CWA) project and included into Autocaliweb
 * to provide duplicate book management functionality.
 *
 * The original code is available at the Calibre-Web Automated GitHub repository:
 * https://github.com/crocodilestick/Calibre-Web-Automated
 *
 * Autocaliweb is also licensed under the GNU General Public License v3.0.
 */
        ...
        if ($(this).hasClass("disabled")) {
            ...
            if (selectedBooks.length === 0) {
                $("#error_modal_message").text(
                    "No books selected! Please select books to delete."
                );
            ...
            var bookIds = selectedBooks.map(function (id) {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    $("#error_modal_message").text(
                        "Error loading book list for confirmation. Status: " +
                            xhr.status +
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    // Show success modal
                    $("#success_modal_message").text(
                        "Selected duplicate books have been deleted successfully!"
                    );
            ...
            error: function (xhr, status, error) {
                ...
                // Show error modal
                $("#error_modal_message").text(
                    "An error occurred while deleting books. Check browser console for details."
                );
                $("#error_modal").modal("show");
            },
        });
