...
$("body > div.container-fluid > div > div.col-sm-10 > div.filterheader").attr(
    "style",
...
if (
    history.length === 1 ||
    curHref[0] + curHref[1] + curHref[2] !=
        prevHref[0] + prevHref[1] + prevHref[2] ||
    $("body.root") > length > 0
) {
...
if ($("body.book").length > 0) {
    ...
    if ($(".identifiers ").length > 0) {
        ...
        if ($(".bookinfo > p:first-child").length > 0) {
            console.log(
                ".bookinfo > p:first-child length " + $(".bookinfo > p").length
            );
    ...
    $(".publishers, .publishing-date, .real_custom_columns, .languages").each(
        function () {
    ...
    if ($(downloads).length === 1) {
        ...
        ).insertBefore(downloads[downloads.length - 1]);
        $(downloads).detach();
        $.each(downloads, function (i, val) {
            $("<li>" + downloads[i].outerHTML + "</li>").appendTo(
                ".leramslist"
            );
        });
    ...
    $(".custom_columns")
        ...
        .prependTo('[aria-label^="Download, send"]');
    $("#have_read_cb").after(
        '<label class="block-label readLbl" for="#have_read_cb"></label>'
    );
    $("#have_read_form").next("p").remove();
    $("#have_read_form").next("p").remove();
    $("#archived_cb").after(
        '<label class="block-label readLbl" for="#archived_cb"></label>'
    );
    ...
    $('ul[aria-labelledby="read-in-browser"]')
        .insertBefore(".blur-wrapper")
        .addClass("readinbrowser-drop");
    $('ul[aria-labelledby="listen-in-browser"]')
        .insertBefore(".blur-wrapper")
        .addClass("readinbrowser-drop");
    $('ul[aria-labelledby="send-to-kereader"]')
        .insertBefore(".blur-wrapper")
        .addClass("sendtoereader-drop");
    ...
    function dropdownToggle() {
        ...
        if ($("#read-in-browser").length > 0) {
            ...
            if (
                position + $(".readinbrowser-drop").width() >
                $(window).width()
            ) {
                positionOff =
                    position +
                    $(".readinbrowser-drop").width() -
                    $(window).width();
                ribPosition = position - positionOff - 5;
                $(".readinbrowser-drop").attr(
                    "style",
                    "left: " +
                        ribPosition +
        ...
        if ($("#sendbtn2").length > 0) {
            ...
            if (
                position + $(".sendtoereader-drop").width() >
                $(window).width()
            ) {
                positionOff =
                    position +
                    $(".sendtoereader-drop").width() -
                    $(window).width();
                ribPosition = position - positionOff - 5;
                $(".sendtoereader-drop").attr(
                    "style",
                    "left: " +
                        ribPosition +
                        "px !important; right: auto; top: " +
        ...
        if ($(".downloadBtn").length > 0) {
            ...
            if (position + $(".leramslist").width() > $(window).width()) {
                positionOff =
                    position + $(".leramslist").width() - $(window).width();
        ...
        if ($('div[aria-label="Add to shelves"]').length > 0) {
            ...
            if (position + $("#add-to-shelves").width() > $(window).width()) {
                positionOff =
                    position + $("#add-to-shelves").width() - $(window).width();
    ...
    $(".book-meta > .bookinfo > .rating")
        .clone()
        .insertBefore(".book-meta > .description")
        .addClass("rating-mobile");
}
...
$(".profileDropli #top_user")
    .parent()
    .after($("#nav_about").addClass("dropdown"));
...
if ($.inArray(bodyClass[0], modalWanted) != -1) {
    ...
    $(" a:not(.dropdown-toggle) ").removeAttr(
        "data-toggle",
        "data-target",
        "data-remote"
    );
...
if ($("body.author").length > 0) {
    ...
    $("#loader + .container-fluid").prepend('<div class="blur-wrapper"></div>');
    $(".blur-wrapper").prepend(
        '<img alt="Blurred author bio" class="bg-blur" src="' + cover + '">'
    );
...
backurl = "../../book/" + url[2];
$("body.epub #title-controls").append(
    '<div class="epub-back"><input action="action" onclick="location.href=backurl; return false;" type="button" value="Back" /></div>'
);
...
$("#acw-switch-theme")
    .attr({
        "data-toggle": "tooltip",
        title: $("#acw-switch-theme").text(), // "Switch Theme"
        "data-placement": "bottom",
...
$("#have_read_cb").click(function () {
    if ($("#have_read_cb:checked").length > 0) {
        $(this).attr(
            "data-original-title",
            $("#have_read_cb").attr("data-checked")
        );
    } else {
        $(this).attr(
            "data-original-title",
            $("#have_read_cb").attr("data-unchecked")
        );
    }
});
...
$("#archived_cb").click(function () {
    if ($("#archived_cb:checked").length > 0) {
        $(this).attr(
            "data-original-title",
            $("#archived_cb").attr("data-checked")
        );
    } else {
        $(this).attr(
            "data-original-title",
            $("#archived_cb").attr("data-unchecked")
        );
    }
});
...
shelfText = $(".shelf .discover h2:first")
    .text()
    .replace(":", " —")
    .replace(/\'/g, "");
$(".shelf .discover h2:first").text(shelfText);

shelfText = $(".shelforder .col-sm-10 .col-sm-6.col-lg-6.col-xs-6 h2:first")
    .text()
    .replace(":", " —")
    .replace(/\'/g, "");
$(".shelforder .col-sm-10 .col-sm-6.col-lg-6.col-xs-6 h2:first").text(
    shelfText
);
...
function mobileSupport() {
    if ($(window).width() <= 768) {
        ...
        $(".row-fluid > .col-sm-2:first").appendTo(
            ".navbar-collapse.collapse:first"
        );
        if ($(".sidebar-backdrop").length < 1) {
            $(".navbar-collapse.collapse:first").after(
                "<div class='sidebar-backdrop'></div>"
            );
        }
    } else {
...
$(".editbook .text-center.more-stuff").prepend(
...
deleteButtons = $(".editbook .text-center.more-stuff a")
...
$.each(deleteButtons, function (i, val) {
    $("<li>" + deleteButtons[i].outerHTML + "</li>").appendTo(
        ".delete-dropdown"
    );
});
