...
def HandleSyncRequest():
    ...
    if only_kobo_shelves:
        ...
        deleted_entries = (
            ...
            .join(ub.KoboSyncedBooks, and_(ub.KoboSyncedBooks.book_id == db.Books.id,
    ...
    for book in books:
        ...
        entitlement = {
            "BookEntitlement": create_book_entitlement(book.Books, archived = book.is_archived==True or (only_kobo_shelves and book.deleted)),
        ...
        if only_kobo_shelves and book.deleted:
            kobo_sync_status.remove_synced_books(book.Books.id)
        else:
...
def get_subtitle(book):
    col = (calibre_db.session.query(db.CustomColumns)
           ...
           .filter(db.CustomColumns.name == 'subtitle')
