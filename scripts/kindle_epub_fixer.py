...
def print_and_log(string, log=True) -> None:
    ...
    if log:
        logger.info(string.replace("[acw-kindle-epub-fixer] ", ""))
    print(string)
...
try:
    ...
    print_and_log("[acw-kindle-epub-fixer] CANCELLING... kindle-epub-fixer was initiated but is already running")
    logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}")
    sys.exit(2)
...
def main():
    ...
    # logger.info(f"ACW Kindle EPUB Fixer Service - Run Started: {datetime.now()}\n")

    ### CATCH INCOMPATIBLE COMBINATIONS OF ARGUMENTS
    if not args.input_file and not args.all:
        print("[acw-kindle-epub-fixer] ERROR - No file provided")
        # logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
        sys.exit(4)
    elif args.all and args.input_file:
        print("[acw-kindle-epub-fixer] ERROR - Can't give all and a filepath at the same time")
        # logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
        sys.exit(5)
    ...
    ### INPUT_FILE PROVIDED
        ...
        if not Path(args.input_file).exists():
            print(f"[acw-kindle-epub-fixer] ERROR - Given file {args.input_file} does not exist")
            # logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
            sys.exit(3)
        if not args.input_file.lower().endswith('.epub'):
            print("[acw-kindle-epub-fixer] ERROR - The input file must be an EPUB file with a .epub extension.")
            # logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
            sys.exit(1)
        ...
        # Run EPUBFixer
        print(f"[acw-kindle-epub-fixer] Processing given file - {args.input_file}...")
        try:
            EPUBFixer(manually_triggered=True).process(args.input_file, output_path, args.language)
        except:
            print(f"[acw-kindle-epub-fixer] ERROR - Error processing {args.input_file}: {e}")
            # logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
            sys.exit(6)
        # logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
        sys.exit(0)
    ...
    ### ALL PASSED AS ARGUMENT
        ...
        logger.info(f"ACW Kindle EPUB Fixer Service - Run Started: {datetime.now()}\n")
        print_and_log("[acw-kindle-epub-fixer] Processing all epubs in library...")
        epubs_to_process = get_all_epubs_in_library()
        if len(epubs_to_process) > 0:
            print_and_log(f"[acw-kindle-epub-fixer] {len(epubs_to_process)} EPUBs found to process.")
            errored_files = {}
            for count, epub in enumerate(epubs_to_process):
                current_position = f"{count + 1}/{len(epubs_to_process)}"
                try:
                    print_and_log(f"\n[acw-kindle-epub-fixer] {current_position} - Processing {epub}...")
                    EPUBFixer(manually_triggered=True, current_position=current_position).process(epub, epub, args.language)
                except Exception as e:
                    print_and_log(f"[acw-kindle-epub-fixer] {current_position} - The following error occurred when processing {epub}:\n{e}")
                    errored_files |= {epub:e}
            if errored_files:
                print_and_log(f"\n[acw-kindle-epub-fixer] {len(epubs_to_process) - len(errored_files)}/{len(epubs_to_process)} EPUBs in library successfully processed")
                print_and_log(f"\n[acw-kindle-epub-fixer] The following {len(errored_files)} encountered errors:\n")
                for file in errored_files:
                    print_and_log(f"   - {file}")
                    print_and_log(f"      - Error Encountered: {errored_files[file]}")
            else:
                print_and_log(f"\n[acw-kindle-epub-fixer] All {len(epubs_to_process)} EPUBs in Library successfully processed! Exiting now...")
        else:
            print_and_log("[acw-kindle-epub-fixer] No EPUBs found to process. Exiting now...")
        logger.info(f"\nACW Kindle EPUB Fixer Service - Run Ended: {datetime.now()}\n")
        sys.exit(0)
