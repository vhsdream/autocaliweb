...
def main():
    ...
    #########################  ENFORCEMENT ARGS  ###########################
        ...
        if enforcer.enforcer_on:
            ...
            print(f"[cover-metadata-enforcer] The ACW Automatic Metadata enforcement service is currently disabled in the settings. Therefore the metadata changes for {log_info['title'].replace(':', '_')} won't be enforced.\n\nThis means that the changes made will appear in the Web UI, but not be stored in the ebook files themselves.")
            enforcer.delete_log()
    else:
        parser.print_usage()
