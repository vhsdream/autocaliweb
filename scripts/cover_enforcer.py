...
try:
    ...
    print(
        "[cover-metadata-enforcer]: CANCELLING... cover-metadata-enforcer was initiated but is already running"
    )
    sys.exit(2)
...
class Book:
    def __init__(self, book_dir: str, file_path: str):
        ...
        self.book_title, self.author_name, self.title_author = (
        ...
        if self.split_library:
            ...
            self.calibre_env["CALIBRE_OVERRIDE_DATABASE_PATH"] = os.path.join(
                self.split_library["db_path"], "metadata.db"
            )
    ...
    def get_new_metadata_path(self) -> str:
        ...
        temp_files = [
            ...
            for (dirpath, dirnames, filenames) in os.walk(metadata_temp_dir)
            for f in filenames
        ]
...
def main():
    ...
    parser.add_argument(
        ...
        dest="log",
        required=False,
        help="Will enforce the covers and metadata of the books in the given log file.",
        default=None,
    )
    parser.add_argument(
        "--dir",
        ...
        dest="dir",
        required=False,
        help="Will enforce the covers and metadata of the books in the given directory.",
        default=None,
    )
    parser.add_argument(
        "-all",
        action="store_true",
        dest="all",
        help="Will enforce covers & metadata for ALL books currently in your calibre-library-dir",
        default=False,
    )
    parser.add_argument(
        "-list",
        ...
        action="store_true",
        dest="list",
        help="List all books in your calibre-library-dir",
        default=False,
    )
    parser.add_argument(
        "-history",
        action="store_true",
        dest="history",
        help="Display a history of all enforcements ever carried out on your machine (not yet implemented)",
        default=False,
    )
    parser.add_argument(
        "-paths",
        ...
        action="store_true",
        dest="paths",
        help="Use with '-history' flag to display stored paths of all files in enforcement database",
        default=False,
    )
    parser.add_argument(
        "-v",
        ...
        action="store_true",
        dest="verbose",
        help="Use with history to display entire enforcement history instead of only the most recent 10 entries",
        default=False,
    )
    ...
    #########################     QUERY ARGS     ###########################
        ...
        and args.dir is None
        and args.all is False
        and args.history is False
    ):
    ...
    #########################  ENFORCEMENT ARGS  ###########################
        ...
        print(
            "[cover-metadata-enforcer]: Enforcing metadata and covers for all books in library..."
        )
        ...
        if os.path.isdir(args.dir):
            ...
            print(
                f"[cover-metadata-enforcer]: ERROR: '{args.dir}' is not a valid directory"
            )
    elif (
        ...
        if enforcer.enforcer_on:
            ...
            if not book_objects:
                print(
                    f"[cover-metadata-enforcer] Metadata for '{log_info['title']}' not successfully enforced"
                )
                sys.exit(1)
            for book in book_objects:
                book.log_info = log_info
        ...
        else:  # Enforcer has been disabled in the ACW Settings
            print(
                f"[cover-metadata-enforcer] The ACW Automatic Metadata enforcement service is currently disabled in the settings. Therefore the metadata changes for {log_info['title'].replace(':', '_')} won't be enforced.\n\nThis means that the changes made will appear in the Web UI, but not be stored in the ebook files themselves."
            )
            enforcer.delete_log()
    else:
        parser.print_usage()
