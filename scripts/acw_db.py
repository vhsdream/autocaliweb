...
class ACW_DB:
    ...
    def temp_disable_split_library(self): # Temporary measure to disable split library functionality until it can be supported in V2.2.0
        ...
        if current_split_setting:
            print("[ATTENTION USER]: Split Libraries (having your books in a separate location to your Calibre Library) are currently unsupported by ACW. This is something currently being worked on to be re-added in V2.2.0")
    ...
    def connect_to_db(self) -> tuple[sqlite3.Connection, sqlite3.Cursor] | None:
        ...
        if con:
            cur = con.cursor()
            if self.verbose:
                print("[acw-db]: Connection with the ACW Enforcement DB Successful!")
            return con, cur


    def make_tables(self) -> tuple[list[str], list[str]]:
        """Creates the tables for the ACW DB if they don't already exist"""
        schema = []
    ...
    def get_acw_default_settings(self):
        for table in self.tables:
            if "acw_settings" in table:
    ...
    def ensure_settings_schema_match(self) -> None:
        self.cur.execute("SELECT * FROM acw_settings")
        acw_setting_names = [header[0] for header in self.cur.description]
        # print(f"[acw-db] DEBUG: Current available acw_settings: {acw_setting_names}")

        # Add any settings present in the schema file but not in the db
        for setting in self.acw_default_settings.keys():
            if setting not in acw_setting_names:
                success = self.add_missing_setting(setting)
                if success:
                    print(f"[acw-db] Setting '{setting}' successfully added to acw.db!")
        
        # Delete any settings in the db but not in the schema file
        for setting in acw_setting_names:
            if setting not in self.acw_default_settings.keys():
                try:
                    print(f"[acw-db] Deprecated setting found from previous version of acw, removing setting '{setting}' from acw.db...")
                    self.cur.execute(f"ALTER TABLE acw_settings DROP COLUMN {setting}")  
                    self.con.commit()
                    print(f"[acw-db] Deprecated setting '{setting}' successfully removed from acw.db!")
                except Exception as e:
                    print(f"[acw-db] The following error occurred when trying to remove {setting} from acw.db:\n{e}")
        ...
        for line in self.schema:
            ...
            if match:
                try:
                    ...
                    print(f"[acw-db] The following error occurred when trying to add {setting} to acw.db:\n{e}")
                    return False
        print(f"[acw-db] Error adding new setting to acw.db: {setting}: Matching setting could not be found in schema file")
        return False
            ...
            if len(current_column_names[table]) < len(column_names_in_schema[table]): # Adds new columns not yet in existing db
                    ...
                    if column_names_in_schema[table][-x] not in current_column_names[table]:
                        for line in self.schema:
                            ...
                            if matches:
                                ...
                                self.con.commit()
                                print(f'[acw-db] Missing Column detected in acw.db. Added new column "{column_names_in_schema[table][-x]}" to table "{table}" in acw.db')
                ...
                for x in range(len(column_names_in_schema[table])):
                    if current_column_names[table][x] != column_names_in_schema[table][x]:
                        ...
                        self.con.commit()
                        print(f'[acw-db] Fixed column mismatch between versions. Column "{current_column_names[table][x]}" in table "{table}" renamed to "{column_names_in_schema[table][x]}"', flush=True)
    ...
    def set_default_settings(self, force=False) -> None:
        ...
        if force:
            for setting in self.acw_default_settings:                    
                if type(self.acw_default_settings[setting]) == int:
                    self.cur.execute(f"UPDATE acw_settings SET {setting}={self.acw_default_settings[setting]};")
                    self.con.commit()
                else:
                    self.cur.execute(f'UPDATE acw_settings SET {setting}="{self.acw_default_settings[setting]}";')
                    self.con.commit()
            print("[acw-db] ACW Default Settings successfully applied!")
            return
        try:
            self.cur.execute("SELECT * FROM acw_settings")
            setting_names = [header[0] for header in self.cur.description]
            ...
            print("[acw-db]: No existing ACW settings detected, applying default ACW settings...")
            for setting in self.acw_default_settings:                    
                if type(self.acw_default_settings[setting]) == int:
                    self.cur.execute(f"UPDATE acw_settings SET {setting}={self.acw_default_settings[setting]};")
                else:
        ...
        for setting in setting_names:
            if setting == "default_settings":
                continue
            elif current_settings[setting] != self.acw_default_settings[setting]:
                default_check = False
                self.cur.execute("UPDATE acw_settings SET default_settings=0 WHERE default_settings=1;")
                self.con.commit()
                break
        if default_check:
            self.cur.execute("UPDATE acw_settings SET default_settings=1 WHERE default_settings=0;")
            self.con.commit()

        if self.verbose:
            print("[acw-db] ACW Settings loaded successfully")


    def get_acw_settings(self) -> dict:
        """Gets the current acw_settings values from the table of the same name in acw.db and returns them as a dict"""
        self.cur.execute("SELECT * FROM acw_settings")
        if self.cur.fetchall() == []: # If settings table is empty, populates it with default values
            self.cur.execute("INSERT INTO acw_settings DEFAULT VALUES;")
            self.con.commit()
    ...
    def update_acw_settings(self, result) -> None:
        """Sets settings using POST request from set_acw_settings()"""
    ...
    def enforce_add_entry_from_log(self, log_info: dict):
        ...
        self.cur.execute("INSERT INTO acw_enforcement(timestamp, book_id, book_title, author, file_path, trigger_type) VALUES (?, ?, ?, ?, ?, ?);", (log_info['timestamp'], log_info['book_id'], log_info['title'], log_info['authors'], log_info['file_path'], 'auto -log'))
        self.con.commit()
    ...
    def enforce_add_entry_from_dir(self, book_dicts: list[dict[str,str]]):
        ...
        for book in book_dicts:
            self.cur.execute("INSERT INTO acw_enforcement(timestamp, book_id, book_title, author, file_path, trigger_type) VALUES (?, ?, ?, ?, ?, ?);", (book['timestamp'], book['book_id'], book['book_title'], book['author_name'], book['file_path'], 'manual -dir'))
            self.con.commit()
    ...
    def enforce_add_entry_from_all(self, book_dicts: list[dict[str,str]]):
        ...
        for book in book_dicts:
            self.cur.execute("INSERT INTO acw_enforcement(timestamp, book_id, book_title, author, file_path, trigger_type) VALUES (?, ?, ?, ?, ?, ?);", (book['timestamp'], book['book_id'], book['book_title'], book['author_name'], book['file_path'], 'manual -all'))
            self.con.commit()


    def enforce_show(self, paths: bool, verbose: bool, web_ui=False):
        results_no_path = self.cur.execute("SELECT timestamp, book_id, book_title, author, trigger_type FROM acw_enforcement ORDER BY timestamp DESC;").fetchall()
        results_with_path = self.cur.execute("SELECT timestamp, book_id, file_path FROM acw_enforcement ORDER BY timestamp DESC;").fetchall()
        if paths:
    ...
    def get_import_history(self, verbose: bool):
        results = self.cur.execute("SELECT timestamp, filename, original_backed_up FROM acw_import ORDER BY timestamp DESC;").fetchall()
        if verbose:
            results.reverse()
            return results
        else:
    ...
    def get_conversion_history(self, verbose: bool):
        results = self.cur.execute("SELECT timestamp, filename, original_format, end_format, original_backed_up FROM acw_conversions ORDER BY timestamp DESC;").fetchall()
        if verbose:
            results.reverse()
            return results
        else:
    ...
    def import_add_entry(self, filename, original_backed_up):
        ...
        self.cur.execute("INSERT INTO acw_import(timestamp, filename, original_backed_up) VALUES (?, ?, ?);", (timestamp, filename, original_backed_up))
        self.con.commit()


    def conversion_add_entry(self, filename, original_format, end_format, original_backed_up): # TODO Add end_format - 22.11.2024 - Done?
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.cur.execute("INSERT INTO acw_conversions(timestamp, filename, original_format, end_format, original_backed_up) VALUES (?, ?, ?, ?, ?);", (timestamp, filename, original_format, end_format, original_backed_up))
        self.con.commit()
    ...
    def get_stat_totals(self) -> dict[str,int]:
        ...
        for table in totals:
            try:
                ...
                print(f"[acw-db] ERROR - The following error occurred when fetching stat totals:\n{e}")
