...
local ACWSync = WidgetContainer:extend{
    name = "acwsync",
    is_doc_only = true,
    title = _("Login to Autocaliweb Server"),
...
function ACWSync:init()
    ...
    self.settings = G_reader_settings:readSetting("ACWSync", self.default_settings)
    ...
    if self.settings.auto_sync and Device:hasSeamlessWifiToggle() and G_reader_settings:readSetting("wifi_enable_action") ~= "turn_on" then
        ...
        logger.warn("ACWSync: Automatic sync has been disabled because wifi_enable_action is *not* turn_on")
    end

    self.ui.menu:registerToMainMenu(self)
end

function ACWSync:getSyncPeriod()
...
function ACWSync:onDispatcherRegisterActions()
    Dispatcher:registerAction("ACWSync_push_progress", { category="none", event="ACWSyncPushProgress", title=_("Push progress from this device"), reader=true,})
    Dispatcher:registerAction("ACWSync_pull_progress", { category="none", event="ACWSyncPullProgress", title=_("Pull progress from other devices"), reader=true, separator=true,})
end

function ACWSync:onReaderReady()
...
function ACWSync:addToMainMenu(menu_items)
    menu_items.progress_sync = {
        text = _("ACW Progress Sync"),
        sub_item_table = {
            {
                text = _("Set Autocaliweb Server"),
                keep_menu_open = true,
                tap_input_func = function()
                    return {
                        -- @translators Server address defined by user for progress sync.
                        title = _("Autocaliweb Server Address"),
...
function ACWSync:setPagesBeforeUpdate(pages_before_update)
    self.settings.pages_before_update = pages_before_update > 0 and pages_before_update or nil
end

function ACWSync:setServer(server)
    logger.dbg("ACWSync: Setting server to:", server)
    self.settings.server = server ~= "" and server or nil
end

function ACWSync:setSyncForward(strategy)
    self.settings.sync_forward = strategy
end

function ACWSync:setSyncBackward(strategy)
    self.settings.sync_backward = strategy
end

function ACWSync:login(menu)
...
function ACWSync:doLogin(username, password, menu)
    local ACWSyncClient = require("ACWSyncClient")
    local client = ACWSyncClient:new{
    ...
    elseif status then
        ...
        UIManager:show(InfoMessage:new{
            text = _("Logged in to ACW server."),
        })
    else
        UIManager:show(InfoMessage:new{
...
function ACWSync:getLastPercent()
...
function ACWSync:getLastProgress()
    if self.ui.document.info.has_pages then
...
function ACWSync:getDocumentDigest()
    return self.ui.doc_settings:readSetting("partial_md5_checksum")
end

function ACWSync:syncToProgress(progress)
    logger.dbg("ACWSync: [Sync] progress to", progress)
    if self.ui.document.info.has_pages then
...
function ACWSync:updateProgress(ensure_networking, interactive, on_suspend)
    ...
    if not interactive and now - self.push_timestamp <= API_CALL_DEBOUNCE_DELAY then
        logger.dbg("ACWSync: We've already pushed progress less than 25s ago!")
        return
    end
    ...
    local ok, err = pcall(client.update_progress,
        ...
        self.device_id,
        function(ok, body)
            logger.dbg("ACWSync: [Push] progress to", percentage * 100, "% =>", progress, "for", self.view.document.file)
            logger.dbg("ACWSync: ok:", ok, "body:", body)
            if interactive then
                if ok then
                    UIManager:show(InfoMessage:new{
...
function ACWSync:getProgress(ensure_networking, interactive)
    if not self.settings.username or not self.settings.password then
        if interactive then
            promptLogin()
        end
    ...
    if not interactive and now - self.pull_timestamp <= API_CALL_DEBOUNCE_DELAY then
        logger.dbg("ACWSync: We've already pulled progress less than 25s ago!")
        return
    end
    ...
    local ok, err = pcall(client.get_progress,
        ...
        doc_digest,
        function(ok, body)
            logger.dbg("ACWSync: [Pull] progress for", self.view.document.file)
            logger.dbg("ACWSync: ok:", ok, "body:", body)
            ...
            local progress = self:getLastProgress()
            local percentage = self:getLastPercent()
            logger.dbg("ACWSync: Current progress:", percentage * 100, "% =>", progress)
...
function ACWSync:_onCloseDocument()
    logger.dbg("ACWSync: onCloseDocument")
...
function ACWSync:schedulePeriodicPush()
...
function ACWSync:_onPageUpdate(page)
    if page == nil then
        return
    end
...
function ACWSync:_onResume()
    logger.dbg("ACWSync: onResume")
...
function ACWSync:_onSuspend()
    logger.dbg("ACWSync: onSuspend")
...
function ACWSync:_onNetworkConnected()
    logger.dbg("ACWSync: onNetworkConnected")
    UIManager:scheduleIn(0.5, function()
...
function ACWSync:_onNetworkDisconnecting()
    logger.dbg("ACWSync: onNetworkDisconnecting")
    -- Network is supposed to be on already, don't wrap this in willRerunWhenOnline
    self:updateProgress(false, false)
end

function ACWSync:onACWSyncPushProgress()
    self:updateProgress(true, true)
end

function ACWSync:onACWSyncPullProgress()
    self:getProgress(true, true)
end

function ACWSync:registerEvents()
    if self.settings.auto_sync then
...
function ACWSync:onCloseWidget()
    UIManager:unschedule(self.periodic_push_task)
